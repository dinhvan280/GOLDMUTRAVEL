                ...
                @if(count($veDat)>0)
                    @foreach($veDat as $id => $cart)
                        <div class="ticket-item">
                            <div class="ticket-item-info">
                                <div class="ticket-header">
                                    <div class="ticket-header-card-title">
                                        <div><h2>{{@$cart['ten_xe']}}</h2><span>[ {{@$cart['ten_chuyen']}} ]</span></div>
                                        <div class="edit-ticket">
                                            {{--                                    <button class="btn-edit"><i aria-hidden="true" class="fa fa-pencil-square-o"></i>--}}
                                            {{--                                    </button>--}}
                                            <button class="btn-remove-ticket" data-id="{{@$cart['ma_cn']}}"><i aria-hidden="true"
                                ...
                                <div class="ticket-content">
                                    ...
                                    <div class="ticket-content-suggest">
                                        <div class="ticket-content-way">
                                            <div><p class="ticket-content-title">Số vé</p>
                                                <p class="ticket-content-info">{{@$cart['so_ve']}}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="ticket-item-price">
                                <div class="price"><p>{{ number_format(floatval(@$cart['tong_tien']), 0, ',', '.') . "₫" }}</p>
                                </div>
                        ...
                        <dialog-message>
                            <div tabindex="-1" role="dialog" class="modal fade modal-custom hide dialog-cart" id="dialog-cart-{{@$cart['ma_cn']}}"
                                ...
                                <div role="document" class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content remove-cart">
                                        ...
                                        <div class="modal-body txt-center"><p class="py-2">Quý khách muốn huỷ vé này?</p></div>
                                        <div class="modal-footer">
                                            <button type="button" data-dismiss="modal" class="btn btn-cancel">Đóng</button>
                                            <button type="button" data-dismiss="modal" class="btn btn-confirm" data-id="{{@$cart['ma_cn']}}">Đồng ý</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </dialog-message>
                        @php $tongTien += @$cart['tong_tien']; @endphp
                    @endforeach
                    <div class="info-total">
                        ...
                        @if(!empty(SESSION('user')))
                            <a href="{{route('tickets.confirm')}}" class="confirm"><button type="button" class="btn-payment">Tiếp
                                    tục</button></a>
                        @else
                            <button type="button" class="btn-payment"><div data-target="#memberModal" class="login nav-info-account btn_login" data-toggle="modal">
                            <span class="login_required">
                                Đăng nhập để xác nhận đặt vé
                        </span>
                                </div></button>
                        @endif
                        <a href="{{route('home')}}" class="backHome"><button type="button" class="btn-payment btn-buy-more">Đặt
                                thêm chuyến khác</button></a>
                ...
                <svg width="160px" height="160px" viewBox="0 0 160 160" version="1.1"
                    ...
                    <g id="mobile" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                        <g id="Group-61" transform="translate(0.397508, 0.036408)">
                            ...
                            <polyline id="Path-8" stroke-opacity="0.502076049" stroke="#242528" stroke-width="4"
                                      points="13 151.013173 122.814093 150.712351 143.602492 150.963592"></polyline>
                        </g>
                    </g>
                </svg>
                <p>Chưa có vé xe nào đang chờ thanh toán</p>
                <a routerlink="/" href="/">Quay lại trang tìm kiếm</a>
            </div>
        @endif
    </div>
</section>
<dialog-message>
    <div tabindex="-1" role="dialog" class="modal fade modal-custom hide" id="dialog-cart"
         style="padding-right: 17px; display: none;">
        <div role="document" class="modal-dialog modal-dialog-centered">
            <div class="modal-content remove-cart">
                <div class="modal-header">
                ...
                <div class="modal-footer">
                    <button type="button" data-dismiss="modal" class="btn btn-cancel">Đóng</button>
                    <button type="button" data-dismiss="modal" class="btn btn-confirm">Đồng ý</button>
                </div>
            </div>
        </div>
    </div>
